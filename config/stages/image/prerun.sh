...
display_alert "Mounting" "BOOT_DEV" "info"
cnt=0
...
display_alert "Root partition" "offset $ROOT_OFFSET, length $ROOT_LENGTH" "info"
...
mount -v "$ROOT_DEV" "${ROOTFS_DIR}" -t ext4 > /dev/null
mkdir -p "${ROOTFS_DIR}/boot"
mount -v "$BOOT_DEV" "${ROOTFS_DIR}/boot" -t vfat > /dev/null
