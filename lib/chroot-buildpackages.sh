...
create_chroot()
{
	...
	[[ ! -f "${target_dir}"/usr/share/keyrings/debian-archive-keyring.gpg ]] && mkdir -p  "${target_dir}"/usr/share/keyrings/ && cp /usr/share/keyrings/debian-archive-keyring.gpg "${target_dir}"/usr/share/keyrings/
	...
	[[ $NO_APT_CACHER != yes ]] && echo 'Acquire::http { Proxy "http://localhost:3142"; };' > "${target_dir}"/etc/apt/apt.conf.d/02proxy
...
chroot_build_packages()
{
	...
	for release in $target_release; do
		for arch in $target_arch; do
			...
			DISTCC_CMDLIST=/tmp/distcc/${release}-${arch}/cmdlist TMPDIR=/tmp/distcc distccd --daemon --pid-file "/var/run/distcc/${release}-${arch}.pid" --listen $distcc_bindaddr --allow 127.0.0.0/24 --log-file "/tmp/distcc/${release}-${arch}.log" --user distccd

			[[ -d $target_dir ]] || exit_with_error "Clean Environment is not visible" "$target_dir"
			...
			if [[ ! -f ${t} || $(( ($(date +%s) - $(<"${t}")) / 86400 )) -gt 7 ]]; then
				...
				(
					tar -cp --directory="${tmp_dir}/" ${t_name} | pv -p -b -r -s "$(du -sb "${tmp_dir}/${t_name}" | cut -f1)" | pixz -4 >"${SRC}/cache/buildpkg/${t_name}.tar.xz"
				)
			fi

			for plugin in "${SRC}"/packages/extras-buildpkgs/*.conf; do
				unset package_name package_repo package_ref package_builddeps package_install_chroot package_install_target package_upstream_version needs_building plugin_target_dir package_component "package_builddeps_${release}"
				source "${plugin}"
...
chroot_installpackages_local()
{
	...
	aptly -keyring="${SRC}/packages/extras-buildpkgs/buildpkg-public.gpg" -secret-keyring="${SRC}/packages/extras-buildpkgs/buildpkg.gpg" -batch=true -config="${conf}" -gpg-key="925644A6" -passphrase="testkey1234" -component=temp -distribution="${RELEASE}" publish repo temp >> "${DEST}"/${LOG_SUBPATH}/install.log
