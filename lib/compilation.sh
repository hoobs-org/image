...
compile_uboot()
{
	...
	while read -r target; do
		...
		if [[ ${BOOTBRANCH} =~ ^tag:v201[8-9](.*) && ${target} != "spi" && -f .config ]]; then
			...
			sed -i 's/^.*CONFIG_ENV_IS_NOWHERE.*/# CONFIG_ENV_IS_NOWHERE is not set/g' .config | echo "# CONFIG_ENV_IS_NOWHERE is not set" >> .config
...
CUSTOM_KERNEL_CONFIG
	...
	if [[ $BRANCH == default ]]; then
		eval CCACHE_BASEDIR="$(pwd)" env PATH="${toolchain}:${PATH}" 'make ARCH=$ARCHITECTURE CROSS_COMPILE="$CCACHE $KERNEL_COMPILER" silentoldconfig'
	else
		eval CCACHE_BASEDIR="$(pwd)" env PATH="${toolchain}:${PATH}" 'make ARCH=$ARCHITECTURE CROSS_COMPILE="$CCACHE $KERNEL_COMPILER" olddefconfig'
	fi
...
apply_patch_series ()
{
	...
	for p in $list
	do
		lsdiff -s --strip=1 "$bzdir/$p" | awk '$0 ~ /^+.*patch$/{print $2}' | xargs -I % sh -c 'rm -f %'
